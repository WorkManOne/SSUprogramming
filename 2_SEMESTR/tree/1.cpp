...
void orderdel(tree* tr, int x) { //(обработка)
	if (tr) {
		orderdel(tr->left, x); //уходим на вывод левого
		orderdel(tr->right, x); //также с правым
		if (tr->inf % x == 0) { //только после рекурсивных переходов начинаем удаление узла, тогда оно начнется с "концов" дерева
			Delete(tr, tr);
		}
	}
}
 //x = 2, 
//5 2 3 6 7 4 12 18 21 9 8 13 10 0
int main() {
	int x;
	cout << "Input x: ";
	cin >> x;
	...
	cout << endl;
	orderdel(n, x); //обрабатываем и удаляем кратные x
	preorder(n); //выводим дерево
	return 0;
}